...
Grid::Grid(const GridConfig& config, double min_x, double min_y, double max_x, double max_y)
    : config_(config) {
    if (config_.step <= 0.0) {
        throw std::runtime_error("网格 step 必须大于 0");
    }
    if (config_.layer_count <= 0) {
        throw std::runtime_error("网格 layer_count 必须大于 0");
    }
...
double Grid::step() const {
    return config_.step;
}

double Grid::min_x() const {
    return min_x_;
}

double Grid::min_y() const {
    return min_y_;
}

double Grid::max_x() const {
    return max_x_;
}

double Grid::max_y() const {
    return max_y_;
}

int Grid::layer_count() const {
    return config_.layer_count;
}

int Grid::x_count() const {
    return x_count_;
}

int Grid::y_count() const {
    return y_count_;
}
