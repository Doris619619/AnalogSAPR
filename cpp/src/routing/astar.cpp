...
// A* 内部状态，方向作为状态的一部分用于计算 bend。
struct SearchState {
    GridPoint point;
    Direction direction = Direction::None;
};

// 优先队列节点，priority 越小越优先。
struct QueueNode {
    SearchState state;
    double priority = 0.0;
};
...
GridPath find_astar_path(
    ...
    if (!grid.in_bounds(start) || !grid.in_bounds(goal)) {
        return GridPath{false, "start 或 goal 超出网格边界", {}, {}};
    }
    ...
    while (!open.empty()) {
        ...
        if (expanded > config.max_expanded_nodes) {
            return GridPath{false, "A* 超过最大展开节点数", {}, {}};
        }
    ...
    return GridPath{false, "A* 未找到可行路径", {}, {}};
}
