...
- `include/sapr/model.hpp`: 公共数据结构。
- `src/io.cpp`: 读写仓库 I/O 格式。
- `src/constraints.cpp`: 约束引用校验和结果指标校验。
- `src/tree.cpp`: 增强 B*-tree、space node、linking-control point 的接口。
- `src/optimizer.cpp`: baseline 求解器。
- `src/router.cpp`: 简单 Manhattan routing、wirelength/bend/via 指标。
- `src/main.cpp`: `validate` 与 `run` CLI。
...
```powershell
build/sapr validate --input input
build/sapr run --input input --output output
```
